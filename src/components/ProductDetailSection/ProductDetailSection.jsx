...
const [descriptionDisplayProperty,setDescriptionDisplayProperty] = useState("")
        ...
        <h3 className="py-4">Size:</h3>
         <div className="flex flex-row gap-6 ml-12">
          {product.size.map((size)=> (
              <p key={product.id} className="bg-zinc-200 p-5">{size}</p>
          ))
}
</div>
 <div className="py-5 mt-10">
          <label for="quantity">Quantity:</label>
            <button onClick={handleDecrease} className="border-2 py-2 px-3 ml-10">-</button>
  <input onChange={handleQuantityChange} id="quantity" value={quantity}  className="appearance-none w-16 py-2 px-5 text-center border border-gray-300 rounded"></input>
   <button onClick={handleIncrease} className="border-2 py-2 px-3">+</button>
...
<div className="flex flex-row mt-20 mb-10 justify-between">
  ...
  {descriptionVisible ? (
   <button onClick={MinimizeDescription} className="pr-10 font-bold">-</button>):
   (<button onClick={MaximizeDescription} className="pr-10 font-bold">+</button>)
  }
</div>
<div className={descriptionDisplayProperty}>
  <p>{product.description}</p>
  <p className="mt-10"><b>Fabric:</b> {product.fabric}</p>
  <p className="mb-16"><b>What You'll Get:</b> {product.content}</p>
  <p className="mb-16"><b>Fit:</b> {product.fit}</p>
  <p className="mb-8"><b>Size&Fit:</b></p>
 ...
 <div className="flex flex-row mt-10 mb-10 justify-between">
  ...
  {deliveryDetailVisible ? (
   <button onClick={MinimizeDeliveryDetail} className="pr-10 font-bold">-</button>):
   (<button onClick={MaximizeDeliveryDetail} className="pr-10 font-bold">+</button>)
  }
</div>
<div className={deliveryProperty}>
 ...
 <hr className="border-black"></hr>
  ...
  {noteVisible ? (
   <button onClick={MinimizeNote} className="pr-10 font-bold">-</button>):
   (<button onClick={MaximizeNote} className="pr-10 font-bold">+</button>)
  }
</div>
